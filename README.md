...
bench/                # reusable Python (no notebook code here)
  client.py           # OpenAI Chat Completions client + timing
...
notebooks/
  ...
  02_memory_footprint.ipynb # the headline bar chart
data/
  prompts.json        # reproducible prompts (text + vision)
  memory_footprint.csv  # appended by notebook 02 across runs
```

The deliverable is intentionally scoped to **memory footprint**, not
latency. A latency comparison across these backends would need every
